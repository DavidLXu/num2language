...
# 最初程序只是想转换德语数字，以后想多扩展几门语言，现在支持德，英，中
# 反向可以用穷举
...
de_units = ['null','tausend','million','milliarde','billion']
	...
	if i == 0:
		...
		if comma == 4:
			if i//1000**4 == 0:
				...
				text += num2de_XXX(i//1000**4)+de_units[comma]
				i %= 1000**4
				comma-=1
		...
		if comma == 3:
			if i//1000**3 == 0:
				...
				text += num2de_XXX(i//1000**3)+de_units[comma]
				i %= 1000**3
				comma-=1
		...
		if comma == 2:
			if i//1000**2 == 0:
				...
				text += num2de_XXX(i//1000**2)+de_units[comma]
				i %= 1000**2
				comma-=1
		if comma == 1:
			if i//1000**1 == 0:
				...
				text += num2de_XXX(i//1000**1)+de_units[comma]
				i %= 1000**1
				comma-=1
		if comma == 0:
			text += num2de_XXX(i)
...
Button(root,text="print entry",command=printentry,fg="blue").pack()
Entry(root,textvariable=var2).pack(fill = BOTH)
