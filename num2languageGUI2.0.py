	...
	if i == 0:
		...
		if comma == 6:
			if i//1000**6 == 0:
				...
				text += en_XXX(i//1000**6)+" "+en_units[comma]+' '
				i %= 1000**6
				comma-=1
		if comma == 5:
			if i//1000**5 == 0:
...
de = Entry(frame_entry,textvariable=var_de,width = 55).pack(side = TOP,fill = BOTH)
en = Entry(frame_entry,textvariable=var_en,width = 55).pack(side = TOP,fill = BOTH)
ch = Entry(frame_entry,textvariable=var_ch,width = 55).pack(side = BOTTOM,fill = BOTH)
root.mainloop()
#print(i,text)
'''
